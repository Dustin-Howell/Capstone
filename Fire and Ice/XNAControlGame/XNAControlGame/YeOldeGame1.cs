...
{
    public class YeOldeGame1 : XNAControl.XNAControlGame, IHandle<MoveRequestMessage>, IHandle<MoveResponseMessage>
    {
        ...
        public YeOldeGame1(IntPtr handle, int width, int height, IEventAggregator eventAggregator)
                ...
                if (_endPostion.Row != -1 && _selectedPeg != null)
                {
                    ...
                    if (_scene.FindName<Nine.Graphics.Model>(_selectedPeg).Visible == true)
                    {
                        if (GameTracker.Board.IsValidMove(move))
                        {
                            ...
                            _eventAggregator.Publish(new MoveResponseMessage(move, PlayerType.Human));
                        }
                        else
                        {
                            _startPosition = new Position(-1, -1);
